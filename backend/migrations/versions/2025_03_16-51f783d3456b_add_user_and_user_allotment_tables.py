...
def upgrade() -> None:
    ...
    sa.CheckConstraint('LENGTH(user_country_code) = 2', name='correct_country_code_format'),
    sa.CheckConstraint('LENGTH(user_email) >= 7', name='min_length_user_email'),
    sa.CheckConstraint('LENGTH(user_first_name) >= 2', name='min_length_user_first_name'),
    sa.PrimaryKeyConstraint('user_id')
    )
    op.create_index(op.f('ix_user_user_email'), 'user', ['user_email'], unique=True)
    op.create_index(op.f('ix_user_user_id'), 'user', ['user_id'], unique=False)
    op.create_table('user_allotment',
    ...
    sa.CheckConstraint('LENGTH(allotment_postal_zip_code) >= 5', name='min_length_allotment_postal_zip_code'),
    sa.CheckConstraint('allotment_length_meters >= 1.0 AND allotment_length_meters <= 100.0', name='check_length_range'),
    sa.CheckConstraint('allotment_width_meters >= 1.0 AND allotment_width_meters <= 100.0', name='check_width_range'),
    sa.ForeignKeyConstraint(['user_id'], ['user.user_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('user_allotment_id'),
    sa.UniqueConstraint('user_id')
    )
    op.create_index(op.f('ix_user_allotment_user_allotment_id'), 'user_allotment', ['user_allotment_id'], unique=False)


def downgrade() -> None:
    """Downgrade schema."""
    op.drop_index(op.f('ix_user_allotment_user_allotment_id'), table_name='user_allotment')
