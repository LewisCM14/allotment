...
describe("EmailVerificationPage", () => {
    ...
    it("shows success message on valid token", async () => {
        ...
        (UserService.verifyEmail as unknown as Mock).mockResolvedValueOnce({ message: "Email verified" });
        const { container } = renderPage();
        await waitFor(() => {
    ...
    it("shows password reset flow if fromReset param is true", async () => {
        ...
        const { container } = renderPage();
        await waitFor(() => {
            expect(screen.getByText(/now you can proceed to reset your password/i)).toBeInTheDocument();
        }, { container });
    ...
    it("shows error message on verification failure", async () => {
        ...
        (UserService.verifyEmail as unknown as Mock).mockRejectedValueOnce(new Error("Invalid or expired token"));
        const { container } = renderPage();
        await waitFor(() => {
    ...
    it("shows request new verification link and return to home links on error", async () => {
        ...
        const { container } = renderPage();
        await waitFor(() => {
            expect(screen.getByRole("link", { name: /request new verification link/i })).toBeInTheDocument();
