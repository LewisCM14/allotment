...
const BASE_API = `${API_URL}${API_VERSION}`;

registerRoute(
	({ url }) =>
		url.href.startsWith(`${BASE_API}/user/auth/`) ||
		url.href.startsWith(`${BASE_API}/user/send-verification-email`) ||
		url.href.startsWith(`${BASE_API}/user/verify-email`) ||
		url.href.startsWith(`${BASE_API}/user/verification-status`),
	new NetworkOnly(),
);

// User profile data
registerRoute(
	({ url }) => url.href.startsWith(`${BASE_API}/user/profile`),
...
registerRoute(
	({ url }) => url.href.startsWith(BASE_API),
	new NetworkFirst({
		cacheName: "api-cache",
		plugins: [
			new ExpirationPlugin({
